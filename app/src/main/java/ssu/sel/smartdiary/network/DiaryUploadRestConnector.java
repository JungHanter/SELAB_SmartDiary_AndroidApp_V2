 ...
 */
public class DiaryUploadRestConnector {
    ...
    public DiaryUploadRestConnector(String apiUrl, String method, OnConnectListener l) {
        this.apiUrl = apiUrl;
        this.method = method.toUpperCase();
        this.listener = l;
    }

    public void request(final File diaryAudioFile, final ArrayList<MediaContext> mediaContextList, final JSONObject json) {
        AsyncTask<Void, Void, JSONObject> reqTask = new AsyncTask<Void, Void, JSONObject>() {
            ...
            protected JSONObject doInBackground(Void... voids) {
                ...
                if (diaryAudioFile != null && diaryAudioFile.exists()) {
                    Log.d("DiaryUpload", diaryAudioFile.getPath());
                    if (diaryAudioFile.exists()) {
                        builder.addPart("file0", new FileBody(diaryAudioFile));
                    }

                    for (int i=0; i<mediaContextList.size(); i++) {
                        MediaContext mediaContext = mediaContextList.get(i);
                        builder.addPart("file"+(i+1), new FileBody(mediaContext.getFile()));
                        Log.d("DiaryUpload", "Media"+(i+1)+": " + mediaContext.getFile());
                    }
                }
